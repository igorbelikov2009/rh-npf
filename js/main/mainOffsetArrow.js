(function () {
  ...
  const column = document.querySelectorAll(".news__column");
  const scrollableElement = document.querySelector(".news__tape");
  ...
  function onLoadCheckForBlurElement() {
    if (screenWidth < 855) {
      column[0].classList.add("news__column_clear");
    } else {
      column[0].classList.add("news__column_clear");
      column[1].classList.add("news__column_clear");
    }
  }
  window.addEventListener("load", onLoadCheckForBlurElement);

  //  меняем прозрачность у "news__column"

  function changeTransparencyColumnArrowLeft() {
    if (scrollWidth < 0) return false;

    for (let i = 0; i < amountChildren; i++) {
      column[i].classList.remove("news__column_clear");
    }

    if (screenWidth > 855) {
      column[q].classList.add("news__column_clear");
      column[q + 1].classList.add("news__column_clear");
    }
